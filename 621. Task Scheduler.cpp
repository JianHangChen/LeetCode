...
// better explaination and sol3 https://leetcode.com/problems/task-scheduler/discuss/104500/java-on-time-o1-space-1-pass-no-sorting-solution-with-detailed-explanation
class Solution {
public:
...
//         for(char task:tasks) count[task - 'A']++;
//         sort(count.begin(), count.end());
//         int mx = 0, maxcount = count[25];
//         for(int i = 25; i >= 0; i--){
//             if(count[i] == maxcount) mx++;
//             else break;
//         }
//         return max((int)tasks.size(), (n+1) * (maxcount - 1) + mx ) ;
